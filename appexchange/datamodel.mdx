...
* A new Thnks is sent, whether from the 'Send a Thnks' iframe button, the Thnks Request/Approval Process workflow, or the Send Thnks Apex Action in Flows (new in v1.9)

* The status of an existing Thnks is changed (e.g. opened, undelivered, redeemed, etc.)

<Note>
  **Send Thnks Apex Action (new in v1.9):** The Apex action sends directly to the Thnks platform via API, bypassing the Thnks Request object and Approval Process entirely, no Thnks Request record is created. The resulting Thnks record in Salesforce is still created through the same organic sync used for web, mobile, and extension activity (see Tracking Thnks Activity below), and goes through the same Contact/Lead/Opportunity matching logic described on this page. This happens asynchronously, the record is not created at the same moment the Flow runs. Separately, the Apex action creates or updates a Task, for its own follow-up (on failure) and completion (on success, via the Source Record Id input), independent of the organic Thnks record's own Task Syncing behavior.
</Note>
...
<Note>
  **Task Syncing vs. Apex Action follow up Tasks (new in v1.9):** The Task Syncing setting above governs Tasks created automatically whenever any Thnks is sent, and is unrelated to the Send Thnks Apex action's own Task behavior. The Apex action instead marks a Flow supplied Source Record Id (often a Task) as complete on success, or creates a separate follow up Task on failure, independent of whether Task Syncing is enabled. See the Install Guide for details on configuring the Apex action.
</Note>
