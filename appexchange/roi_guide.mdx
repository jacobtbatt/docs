...
Demonstrating the ROI of your Thnks investment is crucial for program success and expansion. This guide provides step-by-step instructions for building reports and dashboards that showcase how Thnks accelerates your sales pipeline, improves meeting show rates, and drives revenue growth. All reports below are built on the Thnks object in Salesforce and work the same regardless of how a Thnks was sent, whether via the 'Send a Thnks' iframe button, the Thnks Request/Approval Process workflow, or the Send Thnks Apex Action in Flows (new in v1.9). Apex action sends reach the Thnks object asynchronously, through the same organic sync used for web, mobile, and extension activity, rather than at the moment the Flow runs, but they are fully reportable using the same matching logic as any other send .
...
<Note>
  **Send Thnks Apex Action and ROI Reporting (new in v1.9)**: Sends triggered by the Send Thnks Apex Action are captured in the same Thnks object reports as any other send. The underlying record is created asynchronously rather than at the moment the Flow runs, so expect a short reporting lag rather than a gap. Populate the optional UTM Campaign Id input on the Apex action to tag automation-driven sends for attribution reporting.
</Note>
